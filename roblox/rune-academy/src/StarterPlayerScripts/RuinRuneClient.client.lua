-- Builds the Runes panel: opened by clicking the Fantasy Ruin's orb
-- (WorldBuilder fires OpenRuinRunesUI at this player once the server
-- confirms the Ruin is actually unlocked). Styled after two references
-- given directly: the overall "title + Close button, grid of tier cards"
-- layout of one game's own Runes menu, and the "tier name -> cost ->
-- boost list" per-card look of another. Only one category here (not
-- tabbed like that reference), so no side tab list - just the 5-tier grid.
--
-- Each tier is strictly linear (buy in order 1-5, RuinRuneHandler) and
-- permanent. A locked tier still shows its own card and name (so players
-- can see there's more to come) but its cost/boost stay hidden behind
-- "Discover the rune" until every tier before it is bought - same
-- progressive-reveal idea as the Upgrade Tree tiles.
...
local openRuinRunesUIEvent = remotes:WaitForChild("OpenRuinRunesUI")
...
-- One color per tier, brightest/rarest last - same idea as the reference
-- image's per-tier color ramp (green -> blue -> ... -> gold).
local TIER_COLORS = {
...
dim.Active = true -- Frames don't block input by default - without this, clicks would pass through to the world behind
...
-- 5 cards, 2 per row (4th row holds just the 5th, left-aligned) - explicit
-- positions rather than a UIGridLayout, so the odd one out lands
-- predictably instead of centering/wrapping oddly.
...
local function renderCard(index: number)
	...
	elseif tierState.reachable then
		...
		card.bodyLabel.Text = ("Grants: %s"):format(tierState.label)
...
openRuinRunesUIEvent.OnClientEvent:Connect(function()
