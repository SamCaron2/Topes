...
-- Feeds the Store UI (StoreClient) - the whole catalog (prices, and
-- whether each pass/the Starter Pack is already owned) in one round trip.
getStoreCatalogFunction.OnServerInvoke = function(player)
	return StoreHandler.getCatalog(player)
end
...
getCelestialShardStateFunction.OnServerInvoke = function(player)
...
-- every multiplier/upgrade Manual pickups already do. Also turns on early
-- for anyone who owns HeadStartPass or the Starter Pack
-- (GamePassBoostHandler.hasEarlyAutoMana) - a Robux head start on this
-- exact same reward, well before naturally reaching Tier 2.
...
task.spawn(function()
	while true do
		...
		for _, player in Players:GetPlayers() do
			if WizardTierHandler.hasAutoMana(player) or GamePassBoostHandler.hasEarlyAutoMana(player) then
